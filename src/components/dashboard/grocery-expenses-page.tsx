...
import {
  collectGroceryExpensesForRange,
  formatGroceryExpenseAmount,
  sumGroceryExpenseAmounts,
} from "@/lib/reports/collect-grocery-expense-lines";
import { downloadGroceryExpensesPdf } from "@/lib/reports/grocery-expenses-pdf";
...
export function GroceryExpensesPage({
  params,
}: {
  ...
  const lines = useMemo(() => {
    ...
    return collectGroceryExpensesForRange(transactions, monthRange.start, monthRange.end);
  }, [monthRange, transactions]);

  const totalAmount = useMemo(() => sumGroceryExpenseAmounts(lines), [lines]);
  ...
  const handleDownloadPdf = async () => {
    ...
    setPdfBusy(true);
    try {
      await downloadGroceryExpensesPdf({
        businessName,
        periodTitle,
        lines,
      });
    ...
    return (
      <div className="glass-panel rounded-[1.625rem] p-8 text-center">
        ...
        <p className="mt-2 text-sm text-[var(--lv-muted-strong)]">
          Expenses export is available for grocery workspaces.
        </p>
      </div>
    );
  }
      ...
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:items-end">
            <label className="text-xs font-semibold text-[var(--lv-muted)]" htmlFor="grocery-expenses-month">
              Month
            </label>
            <input
              id="grocery-expenses-month"
              type="month"
              max={maxMonthInput}
              value={monthInput}
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          ...
          >
            {pdfBusy ? "Generating PDF…" : "Download expenses (PDF)"}
          </PressableButton>
          {!txLoading && !txError ? (
            <p className="text-sm text-[var(--lv-muted-strong)]">
              ...
              <span className="font-semibold text-[var(--lv-heading)]">
                {formatGroceryExpenseAmount(totalAmount)}
              </span>
            </p>
          ) : null}
        </div>
              ...
              {lines.map((row, index) => (
                ...
                >
                  ...
                  <td className="lv-tabular-mono px-4 py-3 text-right font-medium text-[var(--lv-heading)]">
                    {formatGroceryExpenseAmount(row.amount)}
                  </td>
                </tr>
              ))}
            </tbody>
